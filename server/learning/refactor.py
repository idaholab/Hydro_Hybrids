...
import matplotlib.pyplot as plt
from helpers.save import savefig, savecsv
from tasks.app import app
import time
from mem_top import mem_top

@app.task()
def model(dict1, energy_profile_path, price_profile_path):
    ...
    capacity_range = dict1['battery']['range_power'] #range of capacities that users want to test
    ...
    efficiency = dict1['battery']['efficiency'] #round-trip efficiency for battery
    uuid = dict1['uuid']


    savecsv(pd.DataFrame(), uuid, f'test_{case_name}')

    #step-2: loading the energy and price profiles
    print(f"{os.getenv('ROOT')}/static/{uuid}/upload/{energy_profile_path}", flush=True)
    print(f"{os.getenv('ROOT')}/static/{uuid}/upload/{price_profile_path}", flush=True)

    datetime = pd.date_range(start='1/1/2019', end='1/1/2020', freq='5Min',closed='left')
    energy_profile_data = pd.read_csv(f"{os.getenv('ROOT')}/static/{uuid}/upload/{energy_profile_path}")
    price_profile_data = pd.read_csv(f"{os.getenv('ROOT')}/static/{uuid}/upload/{price_profile_path}")
    ...
    capacity_interval_range = [ round(elem, 2) for elem in list(np.linspace(capacity_range[0], capacity_range[1], num=capacity_interval)) ]
    ...
    for capacity in range(0,capacity_interval):
        for interval in range(0,storage_hour_interval):
            energy_df = energy_df.append([capacity_interval_range[capacity]*storage_hour_range[interval]])
    ...
    def file_create(energy_profile_data, PC, PP, Cap, E, min_charge, efficiency, gp):
        Co_name= ['Total Hydro Generation (MWh)','Datetime','Price: Energy ($/MWh)', 'Price: RegUp ($/MWh)','Price: RegDn ($/MWh)', 'Price: Spin ($/MWh)', 'Price: NonSpin ($/MWh)']
        file = pd.DataFrame()
    ...
    for i in range(0,con_n):
        df = file_create(energy_profile_data, plant_capacity, price_profile_data, Con_range.iloc[i,1], Con_range.iloc[i,0], min_charge, efficiency, Con_range.iloc[i,2])
    ...
    df_combine['Revenue: Hydro Energy to Grid ($)_cal'] = df_combine['Total Hydro Generation (MWh)_excs']*df_combine['Price: Energy ($/MWh)']/12
    df_combine['Revenue: Hydro Energy to Grid ($)_left_max'] = df_combine['Total Hydro Generation (MWh)']*df_combine['Price: Energy ($/MWh)']/12 - df_combine['Revenue: Hydro Energy to Grid ($)_cal'] 
    ...
    model_name='ann{0}_group_{1}_hour_adam_production_add_2_A_1.h5'.format(model_num,hours)
    model=load_model(os.path.join(current_file+"/model/"+model_name))

    X_sc=load(open(f'{current_file}/model/X_{hours}_scaler_add_2_A_1.pkl','rb'))
    y_sc=load(open(f'{current_file}/model/y_{hours}_scaler_add_2_A_1.pkl','rb'))
    ...
    X,X_index,groups=tools.buildTestWindows_groups_inference(X_df,X_features,segs)
    ...
    all_data['Revenue only'] = all_data['Total Hydro Generation (MWh)']*all_data['Price: Energy ($/MWh)']/12

    rev_cal = all_data.copy()
    rev_cal['group'] = all_data['group']
    rev_cal['hydro_rev_noES'] = rev_cal['Total Hydro Generation (MWh)']*rev_cal['Price: Energy ($/MWh)']/12
    rev_cal['Revenue: Storage Energy to Grid ($)_max'] =  rev_cal['Total Hydro Generation (MWh)_left']*rev_cal['Price: Energy ($/MWh)']/12
    rev_cal['Revenue: Storage RegUp ($)_max'] = rev_cal['Capacity-Power (MW)']*2*rev_cal['Price: RegUp ($/MWh)']/12
    rev_cal['Revenue: Storage RegDn ($)_max'] = rev_cal['Capacity-Power (MW)']*2*rev_cal['Price: RegDn ($/MWh)']/12
    rev_cal['Revenue: Storage Spin ($)_max'] = rev_cal['Capacity-Power (MW)']*2*rev_cal['Price: Spin ($/MWh)']/12
    rev_cal['Revenue: Hydro Energy to Grid ($)_left_max'] = rev_cal['Total Hydro Generation (MWh)']*rev_cal['Price: Energy ($/MWh)']/12 - rev_cal['Revenue: Hydro Energy to Grid ($)_cal'] 

    rev = ['Revenue: Hydro Energy to Grid ($)_cal','Revenue only']
    rev_hydro =pd.DataFrame(columns=rev)
    rev_hydro['Revenue: Hydro Energy to Grid ($)_cal'] = rev_cal.groupby(by=["group","month_day"])['Revenue: Hydro Energy to Grid ($)_cal'].sum()
    rev_hydro['Revenue only'] = rev_cal.groupby(by=["group","month_day"])['Revenue only'].sum()
    ...
    #daily revenue
    combine['Revenue: Hydro Energy to Grid ($)_left'] = combine['Revenue: Hydro Energy to Grid ($)_left'].astype(float).round(2)
    combine['Revenue: Hydro Energy to Grid ($)_cal'] = combine['Revenue: Hydro Energy to Grid ($)_cal'].astype(float).round(2)
    combine['Revenue: Storage Energy to Grid ($)'] = combine['Revenue: Storage Energy to Grid ($)'].astype(float).round(2)
    combine['Revenue: Storage RegUp ($)'] = combine['Revenue: Storage RegUp ($)'].astype(float).round(2)
    combine['Revenue: Storage RegDn ($)'] = combine['Revenue: Storage RegDn ($)'].astype(float).round(2)
    combine['Revenue: Storage Spin ($)'] = combine['Revenue: Storage Spin ($)'].astype(float).round(2)

    combine['Rev_total'] = combine['Revenue: Hydro Energy to Grid ($)_left']+combine['Revenue: Hydro Energy to Grid ($)_cal']+combine['Revenue: Storage Energy to Grid ($)']+combine['Revenue: Storage RegUp ($)']+combine['Revenue: Storage RegDn ($)']+combine['Revenue: Storage Spin ($)']

    # Predicted Daily Revenue Data
    savecsv(combine, uuid, f'PredictedDailyRevenueData_{case_name}')
    #combine.to_csv('Predicted daily revenue data_{0}.csv'.format(case_name))    

    #annual revenue
    preds_year=pd.DataFrame()
    preds_year['Total_Rev_Predicted'] = combine.groupby(['group'])['Rev_total'].sum()
    preds_year['Revenue: Hydro Only'] = combine.groupby(['group'])['hydro_rev_noES'].sum()
    ...
    model_name='ann{0}_group_{1}_hour_adam_production_degd.h5'.format(model_num,hours)
    model=load_model(os.path.join(current_file+"/model/"+model_name))
    ...
    preds_degd_df=pd.DataFrame(preds_degd_orig,columns=y_features,index=X_index)
    ...
    preds_degd_days.loc[:,["group"]] = list(zip(*preds_degd_days.index.values))[0]
    preds_degd_days.loc[:,["Day"]] = list(zip(*preds_degd_days.index.values))[1]
    preds_degd_days = preds_degd_days.reset_index(drop=True)

    # Predicted Daily Battery Degredation
    savecsv(preds_degd_days, uuid, f'PredictedDailyBatteryDegradation_{case_name}')    
    # preds_degd_days.to_csv('Predicted daily battery degradation_{0}.csv'.format(case_name))


    preds_degd_year=pd.DataFrame()
    preds_degd_year['LOSS (%)'] = preds_degd_days.groupby(['group'])['LOSS (%)'].sum()
    ...
    savecsv(cb_degd_plot, uuid, f'PredictedAnnualBatteryDegradation_{case_name}')
    ...
    cb_plot['Net_rev_ES'] = cb_plot['Total_Rev_Predicted']- cb_plot['Revenue: Hydro Only']
    ...
    z_surf = np.array([z_act]*75)             # ex. function, which depends on x and y
    ax.plot_surface(x_surf, y_surf, z_surf, alpha=0.5, rstride=100, cstride=100,color = 'g' );    # plot a 3d surface plot
    ...
    ax.text(E_mark_REV, Cap_mark_REV, max_REV*1.35, "Optimal annual net revenue: ${0}".format(max_REV.round(2)), color='blue')
    ax.text(E_mark_REV, Cap_mark_REV, max_REV*1.25, "E = {0} MWh, Cap = {1} MW".format(max_REV_E.round(2), max_REV_Cap.round(2)), color='blue')
    ax.text(E_mark_REV, Cap_mark_REV, z_act[2]*1.4, "Hydro-only revenue: ${0}".format(z_act[2].round(2)), color='green')


    ax.view_init(azim=160, elev=5)
    # Revenue Plot
    savefig(plt, uuid, f'RevenuePlot_0_{case_name}')
    ...
    def ROI_cal_5(x, y, Ann_rev, degd, gp_n):
        year = 5
        ROI_df = pd.DataFrame({'group_number': gp_n+1, 'E': x,'Cap':y}, index = [gp_n])
        Ann_Rev = Ann_rev
        Cap_c = (x)*(cap_c_kw*(1+degd/100)*year + cc_c)*1000*unit + y*(P_C_c+ B_P_c)*1000*unit #Total Project Costs: Capital Investment Cost ($) - entire cascade (4 units)
        ...
        Ann_depre_b = (x)*cap_c_kw*1000*unit/battery_lifespan #annual depreciation cost of battery
        Annual_deprec_other = (Cap_c - x*cc_c*1000*unit - cap_c_kw*x*1000*unit)*(1-dep_rate)/life_other #annual depreciation cost for other equipment 
        T_Cost = OM_fix + Ann_depre_b + Annual_deprec_other #Total cost= Annual O&M fixed cost + Annual O&M Variable + annual depreciation cost for battery and other equip.
    ...
    for g_n in range(0,group_n):
        ROI_gp = ROI_cal_5(cb_plot.loc[g_n,'E'], cb_plot.loc[g_n,'Cap'], cb_plot.loc[g_n,'Net_rev_ES'], cb_degd_plot.loc[g_n,'LOSS (%)'],g_n)
    ...
    ax.text(E_mark, Cap_mark, max_ROI*1.1, "Optimal 5-year ROI%: {0}".format(max_ROI.round(2)), color='green')
    ax.text(E_mark, Cap_mark, max_ROI*1, "E = {0} MWh, Cap = {1} MW".format(max_ROI_E.round(2), max_ROI_Cap.round(2)), color='green')
    ...
    #plt.show()
    # ROI Plot
    savefig(plt, uuid, f'ROIPlot_0_{case_name}')
    # plt.savefig('ROI plot_0_{0}.png'.format(case_name))
    ...
    def pay_back_p(x, y, Ann_rev, degd, gp_n):
        PBP_df = pd.DataFrame({'group_number': gp_n+1, 'E': x,'Cap':y}, index = [gp_n])
        Ann_Rev = Ann_rev
        Cap_c = (x)*(cap_c_kw*(1+degd/100) + cc_c)*1000*unit + y*(P_C_c+ B_P_c)*1000*unit #Total Project Costs: Capital Investment Cost ($) - entire cascade (4 units)
        ...
        Ann_cash_flow = E_with_tax + Annual_deprec_other + Ann_depre_b#Cash from Operating Activities
    ...
    for g_n in range(0,group_n):
        PBP_gp = pay_back_p(cb_plot.loc[g_n,'E'], cb_plot.loc[g_n,'Cap'], cb_plot.loc[g_n,'Net_rev_ES'], cb_degd_plot.loc[g_n,'LOSS (%)'],g_n)
    ...
    savecsv(cb_plot, uuid, f'PredictedAnnualFinancialPerformance_{case_name}')
    ...
    #ax.scatter3D(x_train, y_train, z_train, c='r', marker='*', s=200)
    plt.title("Pay back period (yrs) vs Energy and Capacity ratings", fontweight='bold')
    #SET x, y, z label
    ax.set_xlabel('Energy (MWh)', labelpad = 10)
    ax.set_ylabel('Capacity-Power (MW)', labelpad = 10)
    ...
    ax.text(E_mark_PBP*0.5, Cap_mark_PBP*0.5, max_PBP*4, "Shortest pay back period (yrs): {0}".format(max_PBP.round(2)), color='green')
    ax.text(E_mark_PBP*0.5, Cap_mark_PBP*0.5, max_PBP*3, "E = {0} MWh, Cap = {1} MW".format(max_PBP_E.round(2), max_PBP_Cap.round(2)), color='green')

    ax.view_init(azim=40, elev=15)
    #plt.show()

    # plt.savefig('Pay back period plot_0_{0}.png'.format(case_name))
    savefig(plt, uuid, f'PaybackPeriodPlot_0_{case_name}')
